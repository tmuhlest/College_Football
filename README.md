...
│
├── data/
│   ├── raw/                  # CFBD parquet snapshots (gitignored)
│   └── processed/            # projected_points.parquet (gitignored)
│
├── models/
│   └── artifacts/            # PyMC posterior (.nc) + projection_constants.json (gitignored)
│
└── notebooks/                # exploratory analysis (gitignored; not part of the package)
    ├── 01_data_exploration.ipynb        # EDA: validate points distributions, snap counts
    └── 02_year_to_year_regression.ipynb # Empirical calibration of regression constants
```
